function new_x = Opt_ETI(M,D,xlower,xupper,GPModels,Batch_size,train_x,train_y)
...
% https://github.com/mobo-d/MOEAD-EGO
   ...
   [pop_ETI,candidate_x,~,~] = MOEAD_ETI(D,xlower,xupper,GPModels,ref_vecs,gmin,z);
...
function  z = get_estimation_z(D, xlower,xupper,GPModels,ref_vecs,z) 
% min(\mu_1(x),...,\mu_m(x))^T
...
% algorithms such as NSGA-II can also be employed.
    ...
    %% the initial population for MOEA/D
    pop_x = (xupper-xlower).*lhsdesign(pop_size, D) + xlower;
    pop_mean = GPEvaluate_mean(pop_x,GPModels);
    ...
    for gen = 1 : maxIter-1
       for i = 1 : pop_size
           ...
           %% generate an offspring and calculate its predictive mean and s
           off_x = operator_DE(pop_x(i,:),pop_x(P(1),:),pop_x(P(2),:), xlower,xupper);    
           [off_mean]= GPEvaluate_mean(off_x,GPModels);  
